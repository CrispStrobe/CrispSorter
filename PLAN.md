...
- [x] **Live test: VPS extraction — audio path** — *2026-05-16*: end-to-end against production cb-api + vps-worker + the C++ `crispasr` binary at `/mnt/storage/whisper.cpp/build/bin/crispasr` (v0.6.6, gcc 13.3.0, Release).  Pushed a 1-s 16 kHz mono WAV (32 KB sine wave) → manifest push + streaming file-by-hash upload → `_extract_via_crispasr` invoked → worker wall time **7.9 s** end-to-end (queue pickup + crispasr cold model load + transcribe + DB update); empty transcript is *correct* for a 440 Hz pure tone with no speech.  Caught + fixed another real bug: `cloud-backup 5d9e4fc fix(extract): CrispASR CLI takes <path> not transcribe; capture via -otxt -of` (the previous code passed a nonexistent `transcribe` subcommand and read stdout, but the CLI is positional and emits transcripts only via `-otxt -of <prefix>` → `<prefix>.txt`).  `CB_CRISPASR_BIN=/mnt/storage/whisper.cpp/build/bin/crispasr` added to `/etc/vps-worker.env`.  Note: `/mnt/storage/whisper.cpp/crispasr/` is a library-only Rust crate, NOT a CLI binary; the actual CLI is the C++ one already present in `build/bin/`.
