...
- **cb-api catalog on `/mnt/volume1`** (2026-05-28) — relocated `<catalog-db>` + `worker_state.db` from the contended VPS root disk (`/dev/sda1`, was 81 % full) to a Hetzner Cloud Volume (live-resizable to 10 TB, POSIX-fsync-safe so SQLite WAL is safe — unlike the CIFS Storage Box).  `/root/cloudworker_state` is now a symlink so anything that hardcoded the old path still resolves.  Wallabag ingest (50 753 markdown rows from `/mnt/storage/wallabag-markdown-export/`) lives in that catalog under `collection_id="wallabag"`.
- **cb-api body-store split → LanceDB** (2026-05-28; cloud-backup's *Stage W*, **not** the same as CrispSorter's Stage W skeleton-index) — body text (`full_text`) routes through the new `api/body_store.py` and lives in the per-shard Lance `documents` table on CIFS (`/mnt/akademie_storage/cb_api_lance/shards/<aa>/`) instead of inline in `file_references`.  Toggled via `CB_BODY_BACKEND=lance` in `/etc/cb-api.env`; `lance` is live on the VPS.  Wire contract unchanged — `cloud_backup.rs::ManifestRow`/`ManifestPullResponse`/`SearchHit` work against both backends with **zero protocol changes**, verified end-to-end via `crispsorter sync cloud-backup pull --include-full-text` returning `{"pulled":3,"applied":3,...}` against the lance-mode VPS.  Catalog on volume1 shrank 1.1 GB → 349 MB post-migration + VACUUM.  Scales the cb-api remote backend toward 5 TB of corpus without bloating volume1; the catalog grows at metadata-only rate (~few hundred bytes/row → ~3 GB at 5 M files).  Test coverage: 53 new unit + 6 new live in cloud-backup (`tests/test_body_store.py` + `tests/test_file_lifecycle.py` + `tests/test_search_edge_cases.py` + `tests/test_real_file_extraction.py` + `tests/test_wallabag_live.py`).
