...
- [~] **Live test: shard backup to WebDAV** — *partial 2026-05-16*: WebDAV transport verified end-to-end against Internxt's built-in WebDAV server (`internxt webdav-start -b` on `http://localhost:9999/`).  Both `webdav_live_*` tests pass — PROPFIND root listing and PUT→STAT→GET→DELETE roundtrip via `WebDavDrive`.  Also verified the `InternxtDrive` direct path (Internxt-Python CLI subprocess) via two new `internxt_live_*` tests; full WRITE→STAT→READ→DEL→STAT-after-delete roundtrip against the live Internxt account.  Real bugs caught and fixed across CrispSorter + cloud-backup + internxt-python (commits `5ab135f`, `9f56cb5`, `7b09898`).  Still pending: the full shard-export → WebDAV-upload → re-download → tarball-sha256 integrity check that exercises `sync::cloud_backup::shard_export_to_drive` against a real drive (the layer above transport).
...
- [~] **Live test: VPS extraction (worker side)** — *partial 2026-05-16*: cb-api Stage V endpoints deployed; vps-worker is now running the new `cloud-backup/vps_worker.py` with the embedded `ExtractionWorker` loop (`/var/log/vps-worker.log` shows the 60-s scan cycle).  cb-api ↔ worker SQLite handshake live-verified: `/api/v2/extract/status` returns `worker_db_found:true` once the worker has created the `pending_extractions` table.  Still blocked from end-to-end image extraction: `CB_CRISPLENS_URL` + `CB_CRISPLENS_SESSION` not yet added to `/etc/vps-worker.env`.  Audio path will graceful-no-op (CrispASR binary not installed on VPS).  Future: populate the two env vars, push a manifest+upload of an image, wait one 60-s scan window, verify `face_count` lands in `<catalog-db>.file_references` via `/api/manifest/pull`.
