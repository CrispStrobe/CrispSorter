...
## Regulatory note — where you may deploy this

CrispSorter runs local AI models to sort, OCR, transcribe, translate and search
documents. Nothing it does is high-risk under the EU AI Act **as shipped**, but
that depends on what you point it at: a *deployer* who applies automatic
document classification to CV screening, credit files, exam material or
...
- **Multimodal Settings + L1/L2/L3 ingest depth** (P13.6 + P13.7) — `Settings → Search Index → Multimodal processing` exposes (a) a master switch + ASR backend dropdown (whisper / whisper-large-v3 / whisper-small / whisper-medium / parakeet / qwen3-omni) + LID method + audio ingest depth (L1=filesystem only / L2=symphonia probe / L3=full transcription, default L3); (b) the parallel image controls — master switch + image ingest depth (L1 / L2=EXIF / L3=EXIF+OCR); (c) an opt-in push to an optional external image service — **deferred, never completed**; see the note under *Photos / Bilder vertical*.  Audio L2 metadata (`duration / codec / sample_rate / channels / bitrate_kbps`) and image L2 EXIF (`camera_make / camera_model / lens_model / taken_at_unix / iso`) land in dedicated LanceDB columns via schema migrations v101 + v102.  Per-row promotion via "Transcribe" (audio) / "Re-OCR" (image) buttons on search results that re-ingest a specific row through the L3 pipeline regardless of the global level.  Drop zones in both Stapel + Kataloge accept all 22 audio/video extensions and surface a "Transcribing" status badge while whisper runs.
...
- **Optional external image service** (opt-in, and behind a cargo feature that is off by default, so it is absent from released builds) — a two-phase hash-dedup then upload path for pushing local images to a self-hosted image service. Off unless you both enable the feature at build time and opt in at runtime.
...
- **Cloud drives** — register **WebDAV** (Nextcloud / ownCloud / mailbox.org / Synology / `filen webdav-start` / `internxt webdav-enable`), **Filen**, **Internxt**, or any local/OS-mounted path as a drive (Quellen → Cloud-Ordner → Anlegen). On iOS and Android, use a reachable remote WebDAV endpoint for Filen/Internxt storage; their local Python CLI daemons cannot run there. Manifest-only L1 ingest of any subtree (no bandwidth cost beyond directory listings); per-row "Promote to L3" downloads + indexes a file's contents on demand via the existing extract+embed pipeline. `crisp+drive://<id>/<remote-path>` URI scheme keeps the same row resolvable across drive renames/edits.
- **Native Filen protocol** — the opt-in `drive-filen-native` feature uses the vendor-aligned Rust client instead of the Python subprocess. It stores the complete encrypted session in the OS keychain, supports Filen v1/v2/v3 crypto, TTL-aware cache listings with fresh-read bypass, recursive `*`/`?`/`**` search, path listings with depth limits, copy/move/rename, replace/update, trash/restore/permanent delete, timestamp metadata, bounded chunk and file concurrency, retries, range downloads, resumable uploads, streamed local path uploads and recursive downloads, streaming progress callbacks, actionable 2FA errors, and cross-client Python↔Rust live round-trips. Configure `TransferConfig` from Rust consumers for chunk size, chunk workers, file workers, retry count, and retry backoff; configure listing TTL separately with `set_listing_cache_ttl`.
...
- **Photos / Bilder vertical** (P13, local features complete) — dedicated "Bilder" tab in Übersicht with image-row filtering (jpg/jpeg/png/webp/heic/heif/tiff/bmp), lazy-loaded thumbnails via IntersectionObserver, click-to-open preview pane with a curated EXIF metadata table (camera, lens, aperture/ISO/exposure, GPS, taken-at), SHA-256 byte-identical dup grouping, and perceptual-hash near-duplicate grouping for resize / re-encode catches. **Deferred: an optional connector to a self-hosted image service.** Partially built, never finished, and deliberately not completed. It sits behind a cargo feature that is off by default and absent from every release build, so shipped artifacts do not contain it and the UI does not offer it. Do not treat it as a supported feature — the remaining work, and the decision to defer, are tracked internally rather than here.

---

## Native Filen development and testing

The native Filen client is opt-in so existing desktop builds remain unchanged:
...
# Mobile compile check: no desktop subprocess or OpenSSL features
cargo check -p crispsorter --no-default-features \
  --features drive-filen-native --target aarch64-apple-ios
```

The live suite is opt-in and creates/cleans unique test folders. Provide
...
The hermetic suite covers gateway response parsing, listing-cache
invalidation, retries, concurrency limits, recursive wildcard matching,
range/resume behavior, mutations, and cryptographic vectors from Filen's MIT
