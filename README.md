...
- **Native Filen protocol** — the opt-in `drive-filen-native` feature uses the vendor-aligned Rust client instead of the Python subprocess. It stores the complete encrypted session in the OS keychain, supports Filen v1/v2/v3 crypto, cache-aware listings, recursive `*`/`?`/`**` search, copy/move/rename, replace/update, trash/restore/permanent delete, timestamp metadata, bounded chunk and file concurrency, retries, range downloads, resumable uploads, reader-based uploads, streaming downloads with progress callbacks, and cross-client Python↔Rust live round-trips. Configure `TransferConfig` from Rust consumers for chunk size, chunk workers, file workers, retry count, and retry backoff.
