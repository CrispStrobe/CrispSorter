...
impl IngestPipeline {
    pub fn new(
        ...
        IngestPipeline { fts, vector, embedder, config }
    }
    ...
    pub async fn ingest_document(&self, raw: RawDocument) -> Result<IngestStats> {
        ...
        for batch in chunks.chunks(self.config.batch_size) {
            ...
            for (i, text_chunk) in batch.iter().enumerate() {
                ...
                let sparse_json = sparse.get(i).and_then(|sv| {
                    sv.as_ref().map(|s| s.to_json().to_string())
                });
        ...
        for batch in all_doc_chunks.chunks(self.config.batch_size) {
            self.vector.ingest_batch(batch).await.context("LanceDB write")?;
        }
        ...
        Ok(IngestStats { chunk_count, embed_time_ms, write_time_ms })
    }
...
fn build_doc_chunk(
    ...
    DocumentChunk {
        ...
        page_count:       None,
        headings_text:    if raw.headings.is_empty() { None } else { Some(raw.headings.join(" ")) },
