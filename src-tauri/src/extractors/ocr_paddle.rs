...
pub fn ocr_via_paddle(path: &Path, rec_lang: OcrRecLang) -> Result<ExtractedDocument> {
    ...
    if region_pairs.is_empty() {
        return Ok(ExtractedDocument { full_text: String::new(), headings: vec![], ext: extension_of(path), language: None, translated_text: None, translated_to_lang: None, audio: None, image_exif: None, source_url: None });
    }
    ...
    Ok(ExtractedDocument { full_text: lines.join("\n"), headings: vec![], ext: extension_of(path), language: None, translated_text: None, translated_to_lang: None, audio: None, image_exif: None, source_url: None })
}
